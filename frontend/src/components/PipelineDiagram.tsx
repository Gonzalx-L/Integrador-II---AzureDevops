...
/**
 * Nodos del pipeline DocuColab — igual al original pero sin Key Vault.
 * Flujo:
 *  Fuente → Orchestrator → Activity → Storage transferencia
 *    → Azure Defender → queue-zip-scan → OnDefenderScanResult
 *      → queue-zip-limpios    → OnCleanZipFromQueue    → documentos/MENSUALES
 *      → queue-zip-protegidos → OnProtectedZipFromQueue → documentos/MENSUALES
 *      → queue-zip-error      → OnErrorZipFromQueue     → MENSUALES/ERROR
 */
...
const EDGES: [string, string, string, string?][] = [
  ...
  ["storage",      "eventgrid",     "#22c55e", "curve"],   // arco decorativo — sin partícula
  ["eventgrid",    "queueScan",     "#22c55e", "curve"],   // arco decorativo — sin partícula
  ["storage",      "queueScan",     "#6366f1"],             // línea principal con partícula
  ...
  ["fnProt",       "storageDest",   "#f59e0b"],
  ["queueError",   "fnError",       "#ef4444"],
  ["fnError",      "storageError",  "#ef4444"],
];

// Edges que son solo decorativos (muestran el arco de Defender)
// pero NO deben tener partículas animadas
const DECORATIVE_EDGES = new Set(["storage-eventgrid", "eventgrid-queueScan"]);
...
const SEGMENT_MAP: Record<string, string> = {
  ...
  "queueProt-fnProt":        "protegidosToStorage",
  "fnProt-storageDest":      "protegidosToStorage",
...
export default function PipelineDiagram({ limpios, protegidos, errores, scanning }: Props) {
  ...
  const arrowId = (c: string) =>
    c === "#22c55e" ? "arrow-green"
    : c === "#f59e0b" ? "arrow-orange"
    : c === "#ef4444" ? "arrow-red"
    : c === "#6366f1" ? "arrow-blue"
    : "arrow-purple";
  ...
  return (
    <div className="pipeline-diagram-wrap">
        ...
        <div className="pipeline-legend">
          ...
          {status && (
            <span style={{fontSize:10,color:"#94a3b8",marginLeft:8}}>
              ⏱ {new Date(status.updatedAt).toLocaleTimeString("es-PE")}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="pipeline-svg-container">
        <svg viewBox="0 0 1020 420" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",height:"auto"}}>
          <defs>
            {[
          ...
          {EDGES.map(([fk,tk,color,curve],i) => {
            ...
            return (
              <path key={i}
                d={getPath({x:f.x+22,y:f.y},{x:t.x-22,y:t.y},curve)}
          ...
          {/* Partículas en segmentos activos — nunca en edges decorativos */}
          {EDGES.map(([fk,tk,color,curve],i) => {
            const segKey  = `${fk}-${tk}`;
            if (DECORATIVE_EDGES.has(segKey)) return null;  // arcos de Defender: sin partícula
            const isActive = seg(SEGMENT_MAP[segKey] || "") || ALWAYS_ACTIVE.includes(segKey);
            if (!isActive) return null;
            const f = NODES[fk], t = NODES[tk];
            return (
              <EdgeParticle key={`p-${i}`}
                x1={f.x+22} y1={f.y} x2={t.x-22} y2={t.y}
                color={color} delay={i*0.25} curve={curve}
              />
            );
          })}
              ...
              <g key={key} transform={`translate(${n.x-22},${n.y-22})`}>
                ...
                {n.label.split("\n").map((line,li) => (
                  <text key={li} x={22} y={56+li*13} textAnchor="middle" fontSize={9}
                    ...
                    fontWeight={active ? 600 : 400}>{line}</text>
                ))}
              </g>
            );
          })}

          {/* Contadores — solo mostrar si hay actividad real */}
          {[
            { x: 940, y: 78,  count: queueLimpios,    bg:"#22c55e22", stroke:"#22c55e", text:"#16a34a" },
            { x: 940, y: 193, count: queueProtegidos,  bg:"#f59e0b22", stroke:"#f59e0b", text:"#b45309" },
            { x: 940, y: 303, count: queueError,       bg:"#ef444422", stroke:"#ef4444", text:"#dc2626" },
          ].filter(c => c.count > 0).map((c,i) => (
            <g key={i}>
              <rect x={c.x} y={c.y} width={38} height={22} rx={6}
                fill={c.bg} stroke={c.stroke} strokeWidth={1.5}/>
              <text x={c.x+19} y={c.y+15} textAnchor="middle"
                fontSize={12} fill={c.text} fontWeight={700}>{c.count}</text>
            </g>
          ))}
        </svg>
      </div>
